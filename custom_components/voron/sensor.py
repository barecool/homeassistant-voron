...
sensor:
  - platform: voron
    ...
    resources:
      - chamber_temperature (optional)
      - chamber_pressure (optional)
      - chamber_humidity (optional)
      - object_layer_height_total (optional)
      - object_layer_height_current (optional)
      - status (optional)
      - state (optional)
      - message (optional)
      - progress (optional)
      ...
      - file_name_metadata (optional)
      - gcode_preview (optional)
      - print
      - bed_temperature (optional)
      - bed_temperature_target (optional)
      - bed_power (optional)
      - bed_type (optional)
      - bed_heaters_power (optional)
      - hotend_1_temperature (optional)
      - hotend_1_temperature_target (optional)
      - hotend_1_temperature_id (optional)
      - hotend_1_power (optional)
      - hotend_1_pressure_advance
      - hotend_1_smooth_time (optional)
      - hotend_2_temperature (optional)
      - hotend_2_temperature_target (optional)
      - hotend_2_temperature_id (optional)
      - hotend_2_power (optional)
      - hotend_2_pressure_advance
      - hotend_2_smooth_time (optional)
      - host_info (optional)
      - state_message (optional)
      - printer_sensor 
      - update_full (optional)
      - update_klipper (optional)
      - emergency_stop (optional)
      - firmware_restart (optional)
      - cancel (optional)
      - pause (optional)
      - resume (optional)
      - restart_klipper (optional)
      - restart_webcam (optional)
      - restart_moonraker (optional)
      - caselight (optional)
      - gcode_command (optional)
...
BASE_URL = "http://{0}:7125/printer/object"


async def async_setup_platform(
    hass: HomeAssistantType, config, async_add_entities, discovery_info=None
):
    """Setup the Voron printer sensors"""
    session = async_get_clientsession(hass)
    data = VoronStatusData(session, config.get(CONF_HOST))
    ...
    if CONF_SENSORS in config:
        for sensor in config[CONF_SENSORS]:
            ...
            _LOGGER.debug(
                f"Adding Voron printer sensor: {name}, {sensor_type}, {unit}, {icon}"
            )
            entities.append(
                VoronStatusSensor(
...
class VoronStatusData(object):
    """Handle Voron object and limit updates"""

    def __init__(self, session, host):
        if host:
            self._url_printer = BASE_URL.format(host) + "/list"
            self._url_print_job = BASE_URL.format(host) + "/query?heater_bed&extruder&print_stats&toolhead&display_status&virtual_sdcard&gcode_move&filament_switch_sensor runout"
            self._url_system = BASE_URL.format(host) + "/info"
        self._host = host
    ...
    async def async_update(self):
        """Download and update data from the Voron Printer"""
        if self._host:
            try:
    ...
    async def fetch_data(self, url):
        try:
            ...
            _LOGGER.error(
                f" Timeout error occurred while polling voron printer using url {url}"
            )
        except Exception as err:
            _LOGGER.error(
                f"Unknown error occurred while polling Voron printer using {url} -> error: {err}"
            )
            return {}
        ...
        try:
            ...
            _LOGGER.error(f"Cannot parse data received from Voron printer {err}")
            return {}
        return ret
...
class VoronStatusSensor(Entity):
    """Representation of a Voron status sensor"""

    def __init__(
        self, data: VoronStatusData, name, sensor_type, unit, icon, decimal
    ):
